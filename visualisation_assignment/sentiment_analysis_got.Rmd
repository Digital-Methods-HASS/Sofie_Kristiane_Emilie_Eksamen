...
```{r split-lines}
ipcc_df <- data.frame(ipcc_text) %>% #Converting text into a data frame
  mutate(text_full = str_split(ipcc_text, pattern = '\n')) %>% #One line become one row separated by '\n'
  unnest(text_full) %>% 
...
ipcc_no_numeric <- ipcc_stop %>% 
  filter(is.na(as.numeric(word))) # All the words wil be given 'NA' and we want to keep them. 
...
# Note: may be prompted to download (yes) d
...
The most common meaningful words is king, lord and sir, and we expect that the emotions will be anger, fear, sad, surprised which it was.

Are there any terms that are similarly ambiguous to the 'confidence' above? 
The term ‘lord’ in the Game of Thrones dataset is similarly ambiguous to the 'confidence' in the first dataset. 


### Get the Game of Thrones book:
```{r get-document for Game of Thrones}
...
- How cool to extract text out of a PDF! Do you think it will work with any PDF?
No, we think it will only work on a text that is OCR-processed.

- Each row is a page of the PDF (i.e., this is a vector of strings, one for each page)
- The pdf_text() function only sees text that is "selectable"

Example: Just want to get text from a single page (e.g. Page 9)? 
```{r single-page for Game of Thrones}
got_p9 <- got_text[9]
...
```{r split-lines for Game of Thrones}
...
```{r tokenize for Game of Thrones}
got_tokens <- got_df %>% 
  unnest_tokens(word, text_full)
...
Let's count the words!
```{r count-words for Game of Thrones}
got_wc <- got_tokens %>% 
  count(word) %>% 
  arrange(-n)
...
We will *remove* stop words using `tidyr::anti_join()`:
```{r stopwords for Game of Thrones}
got_stop <- got_tokens %>% 
  anti_join(stop_words) %>% 
  select(-got_text)
...
Now check the counts again: 
```{r count-words2 for Game of Thrones}
got_swc <- got_stop %>% 
  count(word) %>% 
  arrange(-n)
...
```{r skip-numbers for Game of Thrones}
...
```{r wordcloud-prep for Game of Thrones}
# There are almost 2000 unique words 
...
```{r wordcloud for Game of Thrones}
...
That's underwhelming. Let's customize it a bit:
```{r wordcloud-pro for Game of Thrones}
...
```{r bind-afinn for Game of Thrones}
got_afinn <- got_stop %>% 
  inner_join(get_sentiments("afinn"))
...
Let's find some counts (by sentiment ranking):
```{r count-afinn for Game of Thrones}
...
```{r afinn-2-more for Game of Thrones}
# Check the unique 2-score words:
...
Look back at the IPCC report, and search for "confidence." Is it typically associated with emotion, or something else? 
It is typically an emotion, but in this context the word "confidence" is associated with a term. The term is used to indicate the assessed likelihood of an outcome or a result.

We learn something important from this example: Just using a sentiment lexicon to match words will not differentiate between different uses of the word...(ML can start figuring it out with context, but we won't do that here).

Or we can summarize sentiment for the report: 
```{r summarize-afinn for Game of Thrones}
...
```{r bind-bing for Game of Thrones}
got_nrc <- got_stop %>% 
  inner_join(get_sentiments("nrc"))
...
```{r check-exclusions for Game of Thrones}
...
Now find some counts: 
```{r count-bing for Game of Thrones}
got_nrc_n <- got_nrc %>% 
  count(sentiment, sort = TRUE)
...
Or count by sentiment *and* word, then facet:
```{r count-nrc for Game of Thrones}
got_nrc_n5 <- got_nrc %>% 
  count(word,sentiment, sort = TRUE) %>% 
  group_by(sentiment) %>% 
  top_n(5) %>% 
...
```{r nrc-confidence for Game of Thrones}
